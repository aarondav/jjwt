        ...
        if (base64UrlEncodedDigest != null) { //it is signed - validate the signature
            ...
            if (key == null) { //fall back to keyBytes
                ...
                if (!Objects.isEmpty(keyBytes)) {
                    ...
                    if (algorithm.isRsa()) {
                        throw new IllegalArgumentException("Key bytes cannot be specified for RSA signatures.  Please specify a PublicKey or PrivateKey instance.");
                    }
                    
                    if (algorithm.isEllipticCurve()) {
                        throw new IllegalArgumentException("Key bytes cannot be specified for ECDSA signatures.  Please specify a PublicKey instance.");
                    }
